...
import (
	...
	"github.com/boknowswiki/boknows_services/service/app/sales-api/handlers"
	...
	"github.com/boknowswiki/boknows_services/service/business/tests"
...
func (ut *UserTests) getToken200(t *testing.T) {
	r := httptest.NewRequest(http.MethodGet, "/users/token/"+ut.kid, nil)
	w := httptest.NewRecorder()
...
func (ut *UserTests) getUser403(t *testing.T) {
	t.Log("Given the need to validate regular users can't fetch other users.")
	{
		...
		{
			r := httptest.NewRequest(http.MethodGet, "/users/"+tests.AdminID, nil)
			w := httptest.NewRecorder()
		...
		t.Logf("\tTest %d:\tWhen fetching the user as themselves.", testID)
		{
			...
			r := httptest.NewRequest(http.MethodGet, "/users/"+tests.UserID, nil)
			w := httptest.NewRecorder()
...
func (ut *UserTests) putUser404(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodPut, "/users/"+id, bytes.NewBuffer(body))
	w := httptest.NewRecorder()
...
func (ut *UserTests) postUser201(t *testing.T) user.Info {
	...
	t.Log("Given the need to create a new user with the users endpoint.")
	{
		...
		t.Logf("\tTest %d:\tWhen using the declared user value.", testID)
		{
			...
			exp := got
			exp.Name = "Bo Tao"
			exp.Email = "boknowswiki@gmail.com"
...
func (ut *UserTests) putUser204(t *testing.T, id string) {
	...
	t.Log("Given the need to update a user with the users endpoint.")
	{
		...
		t.Logf("\tTest %d:\tWhen using the modified user value.", testID)
		{
			...
			if ru.Email != "boknowswiki@gmail.com" {
				t.Fatalf("\t%s\tTest %d:\tShould not affect other fields like Email : got %q want %q", tests.Failed, testID, ru.Email, "bill@boknowswiki/boknows_services.com")
			}
